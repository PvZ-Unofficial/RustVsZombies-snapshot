...
fn install_hooks() {
    ...
    on_enter_fight(|| {
        let result = rsvz::with_backend(|backend| -> rsvz::runtime::RuntimeResult<()> {
            let selection = CardSelection::Plant(PlantKind::SnowPea)
                .checked()
                .expect("SnowPea is a valid card selection");
...
fn script() -> rsvz::runtime::RuntimeResult<()> {
    ...
    let mut repro = witness::WitnessRepro {
        case_id: "snow-pea-r3c3-vs-natural-normal-locked-mid-frames3500".to_owned(),
        scenario_seed: Some(0x5eed_1051),
        script_id: "witness-snow-pea-natural-normal-v1".to_owned(),
    ...
    repro
        .set_expanded_setup_json(
            r#"{
                ...
                "plants": [{"kind": "snow_pea", "row": 3, "col": 3}],
                "zombies": [],
                "spawn": {
                    "mode": "natural",
                    "wave_1": ["normal"]

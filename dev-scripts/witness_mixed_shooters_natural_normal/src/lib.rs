...
fn install_hooks() {
    ...
    on_enter_fight(|| {
        let result = rsvz::with_backend(|backend| -> rsvz::runtime::RuntimeResult<()> {
            for (row, kind) in [
                (0, PlantKind::Repeater),
                (1, PlantKind::GatlingPea),
                (2, PlantKind::FumeShroom),
                (3, PlantKind::CabbagePult),
                (4, PlantKind::PuffShroom),
            ] {
                let selection = CardSelection::Plant(kind)
                    .checked()
                    .expect("ordinary plants are valid card selections");
                let grid = Grid::new(row, 2).expect("R1C3 through R5C3 are valid grids");
...
fn script() -> rsvz::runtime::RuntimeResult<()> {
    ...
    let mut repro = witness::WitnessRepro {
        case_id: "mixed-shooters-r1-r5-c3-natural-normal-private-mt-frames4000".to_owned(),
        scenario_seed: Some(0x5eed_1051),
        script_id: "witness-mixed-shooters-natural-normal-v1".to_owned(),
    ...
    repro
        .set_expanded_setup_json(
            r#"{
                ...
                "plants": [
                    {"kind": "repeater", "row": 1, "col": 3},
                    {"kind": "gatling_pea", "row": 2, "col": 3},
                    {"kind": "fume_shroom", "row": 3, "col": 3},
                    {"kind": "cabbage_pult", "row": 4, "col": 3},
                    {"kind": "puff_shroom", "row": 5, "col": 3}
                ],
                "zombies": [],
                "spawn": {
                    "mode": "natural",

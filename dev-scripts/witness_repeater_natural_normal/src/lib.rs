...
fn install_hooks() {
    ...
    on_enter_fight(|| {
        let result = rsvz::with_backend(|backend| -> rsvz::runtime::RuntimeResult<()> {
            let selection = CardSelection::Plant(PlantKind::Repeater)
                .checked()
                .expect("Repeater is a valid card selection");
...
fn script() -> rsvz::runtime::RuntimeResult<()> {
    ...
    let mut repro = witness::WitnessRepro {
        case_id: "repeater-r1c3-natural-normal-private-mt-frames1000".to_owned(),
        scenario_seed: Some(0x5eed_1051),
        script_id: "witness-repeater-natural-normal-v1".to_owned(),
    ...
    repro
        .set_expanded_setup_json(
            r#"{
                ...
                "plants": [{"kind": "repeater", "row": 1, "col": 3}],
                "zombies": [],
                "spawn": {
                    "mode": "natural",
                    "wave_1": ["normal"]

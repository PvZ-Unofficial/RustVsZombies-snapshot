//! FE 后退无炮浅梦解 20250601, with the agreed planting and emergency fixes.
use PlantKind::*;
use ZombieKind as Z;
...
use rsvz::core::model::GridExplosionKind;
...
fn play(card: CardSelection, pos: Pos) -> bool {
    ...
    if !reject(card, pos).is_allowed() {
        return false;
    }
    matches!(try_plant_seed(slot, grid(pos)), PlantSeedOutcome::Planted(_))
}
...
fn deal_bungee() -> RuntimeResult<()> {
    if !usable(sel(IceShroom)) && !usable(CardSelection::Imitator(IceShroom)) {
        later(400, || {
            for (row, positions) in [(3, &[(3, 8), (3, 9)][..]), (4, &[(4, 8), (4, 9)][..])] {
                if count(Z::Bungee, row, 0, f32::NEG_INFINITY, f32::INFINITY, 1) > 0 {
                    recover(sel(Jalapeno), positions)?;
                }
            }
            Ok(())
        })?;
...
fn fix_glooms() -> RuntimeResult<()> {
    if sun() < 1000 || !usable(sel(FumeShroom)) || card_cd(sel(GloomShroom)) != Some(0) {
        return Ok(());
    }
...
fn start_fight() -> RuntimeResult<()> {
    rsvz::log(
        rsvz::LogLevel::Debug,
        format_args!(
            "houtui_open epoch={} rounds={} strategy={:?} sun={}",
    ...
    if std::env::var("HOUTUI_TRACE_LOSS").as_deref() != Ok("0") {
        for wave in 1..=20 {
            at(wave, 0, || {
                rsvz::log(
                    rsvz::LogLevel::Debug,
...
fn script() -> RuntimeResult<()> {
    if EPOCH.get().is_none() {
        if let Ok(directory) = std::env::var("HOUTUI_TRACE_DIR") {
    ...
    let seconds = setting("HOUTUI_SECONDS", 600);
    if seconds != 0 {
        rsvz::measure::expected_passes_for_with_end(
            Duration::from_secs(seconds),
            rsvz::WorldResetConfig {
                initial_sun: setting("HOUTUI_SUN", 8000) as u32,
                completed_rounds: setting("HOUTUI_ROUNDS", 500) as u32,
                ..Default::default()
            },
            if std::env::var("HOUTUI_FINISH_ACTIVE").as_deref() == Ok("0") {
